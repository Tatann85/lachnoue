...
export function build(om, cal) {
  const H = {};
  om.hourly.time.forEach((t, i) => { H[t.slice(0, 13)] = { s: om.hourly.wind_speed_10m[i], g: om.hourly.wind_gusts_10m[i], d: om.hourly.wind_direction_10m[i] }; });
  const dates = om.daily.time;
  const es = ecluseStates(cal, dates[0], dates[dates.length - 1]);
  const days = dates.map((date, i) => {
    const wind = HS.map(h => { const k = H[date + 'T' + pad(h)] || {}; return [h, Math.round(k.s || 0), Math.round(k.g || 0), Math.round(k.d || 0)]; });
